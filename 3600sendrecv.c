...
/**
 * This function takes a header as datum and its data and
 * computes its 8-bit checksum.
 */
unsigned char get_checksum(char *datum, char *data, int length) {
	// number of 8-bit chunks in header
	size_t num_datum = sizeof(header) / sizeof(char);
	// sum all chunks in header
	unsigned short sum = 0;
	for (size_t i = 0; i < num_datum; i++) {
		sum += *datum++;
		// check for carry bit (9th bit is 1)
		if (sum & 0xff00) {
			// drop off that 9th bit
			sum &= 0xff;
			// add that 1 bit to rightmost bit
			sum++;
		}
	}

	// sum all chunks is data
	num_datum = length;
	for (size_t i = 0; i < num_datum; i++) {
		sum += *data++;
		// check for carry bit (9th bit is 1)
